...
async function generateWord(review) {
  if (!window.docx) {
    await new Promise((resolve, reject) => {
      ...
      s.src = 'https://unpkg.com/docx@9.6.1/build/index.umd.js';
      s.onload = resolve; s.onerror = reject;
      document.head.appendChild(s);
    });
  }
  const { Document, Packer, Paragraph, TextRun, Table, TableRow, TableCell,
    AlignmentType, BorderStyle, WidthType, ShadingType, Header, Footer } = window.docx;
  ...
  children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before:0, after:80 },
    children: [new TextRun({ text:`Perspective: ${(review.party||'').toUpperCase()}  |  Generated: ${new Date().toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})}`, font:'Arial', size:20, color:'888888', italics:true })] }));
  children.push(new Paragraph({ spacing:{before:100,after:400}, border:{bottom:{style:BorderStyle.SINGLE,size:6,color:NAVY,space:1}}, children:[] }));
